...
function VerifyEmail({ token, email }) {
  const { verifyUser, resend, loading, error, verified, message } =
    useVerifyStore((state) => ({
        ...
        <div className="flex gap-4 md:w-[479px] w-[90%] flex-col items-start justify-start">
          ...
          {loading && (
            <div className="flex items-center justify-center mx-auto w-full">
              <div className="w-20 h-20 border-4 border-sky-700 border-dotted rounded-full animate-spin"></div>
            </div>
          )}
        </div>
      ...
      {!loading && (
        <Link className="w-full md:w-[481px]" href="/signin">
          <div className="md:w-[481px] w-[90%] mt-[10px] h-11 px-5 py-3  justify-center items-center gap-2 inline-flex">
            <img
              src="/images/auth/arrow_left.svg"
              alt=""
              className="w-6 h-6 "
            />
...
function Success({ message }) {
  return (
    <>
      ...
      <p className="text-center w-full font-semibold">{message}</p>
    </>
  );
}
...
function Error({ message }) {
  return (
    <>
      <div className="flex items-center justify-center mx-auto w-full">
        <div className="bg-red-500 rounded-full p-2 animate-fade-in inline-flex items-center justify-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
