...
createChatbot.schema = joi.object({
  ...
  include: joi.array().items(joi.string().uri({ scheme: ['http', 'https'] })).required(),  //array of string
  exclude: joi.array().items(joi.string().uri({ scheme: ['http', 'https'] })).required()  //array of string
});
